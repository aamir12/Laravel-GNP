 ...
 * @group Admin API - KPI
 *
 * Admin APIs for magaging KPI data.
 *
...
class AdminKPIController extends Controller
{
     ...
     *
     * @responseFile 200 resources/responses/Admin/KPI/create.json
     * @responseFile 422 resources/responses/Admin/KPI/create-422.json
     */
    public function create(BulkCreateKpiRequest $req)
    {
     ...
     * List KPI Data
     *
     * Lists the specified user's KPI data.
     *
     * @bodyParam user_id int Identifies which user to list KPI for. If omitted, will list KPI for all users. Example: 1
     *
     * @responseFile 200 resources/responses/Admin/KPI/list.json
     */
    public function list(Request $req)
    {
        $req->validate(['user_id' => 'exists:users,id']);
        $kpi = KpiRepository::listKpiDataForUser($req->user_id ?? NULL, true);
        return response()->success(__('score')['kpi_list_success'], $kpi, true);
    }
}
